    ...
    if len(os.listdir(f'{os.path.dirname(os.path.abspath(__file__)).replace("\\","/")}/uploadedfiles'))!= 0 :
        ...
        for file in allfiles:
            if ".dat" in file:
                ...
                df["T_A[K]"] = df["T_A[K]"].astype(float)
                datfiles.append(pd.read_csv(f"{path}/{file}",sep=" "))
        results = pd.concat(datfiles)
        sorted = results.sort_values("T_A[K]",ascending=False)
...
def renderresults():
    ...
    if len(os.listdir(f"{generalpath}/sorted")) == 1:
        ...
        sensors = [ i for i in data.columns if '[Ohm]' in i and (not '_A' in i or not'_B' in i)]
        for sensor in sensors:
            plt.scatter(data.loc[:,"T_A[K]"],data.loc[:,sensor],c="g",s=5,label = "Measured values")
            a = np.linspace(min(data.loc[:,"T_A[K]"]),max(data.loc[:,"T_A[K]"]),5000)
            fitted = scipy.interpolate.interp1d(data.loc[:,"T_A[K]"],data.loc[:,sensor],fill_value="extrapolate")
            ...
            values : list
            limits = (math.floor(max(data.loc[:,"T_A[K]"])),min(data.loc[:,"T_A[K]"]))
            if limits[0] >100:
                if limits[1] < 4:
            ...
            logValues = ["{:.9f}".format(np.log10(i)) for i in extrapolated_values ]
